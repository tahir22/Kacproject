...
{
    public class WebsocketnewController : ApiController
    {
        ...
        public HttpResponseMessage Gett()
        {
           ...
           //Console.WriteLine("Server has started on {0}:{1}, Waiting for a connection...", ip, port);
            ...
            while (true)
            {
            while (!streamm.DataAvailable) ;
                while (client.Available < 3) ; // match against "get"

                byte[] bytes = new byte[client.Available];
                streamm.Read(bytes, 0, client.Available);
                ...
                else
                {
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK , "mask bit not set");
                }
            }
